...
function App() {
  ...
  const [tempUnSelectedRange, setTempUnSelectedRange] = useState({ from: null, to: null });
  ...
  }, [tempUnSelectedRange]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const modifiers = {
    ...
    unSelected: (day) => DateUtils.isDayInRange(day, { from: tempUnSelectedRange.from, to: tempUnSelectedRange.from ? lastDayUnselectedMouseEnter : null }),
    from: getFromDate(),
    to: getToDate(),
  };
  ...
  return (
    <React.Fragment>
      ...
      <StyledContainer>
        <StyledFilterContainer>
          <Select options={yearOptions} value={selectedYear} onChange={setSelectedYear} />
        ...
        <StyledCalendarContainer>
          {[...Array(12)].map((test, index) => {
            const selectedDays = [
              ...
              { from: tempUnSelectedRange.from, to: tempUnSelectedRange.from ? lastDayUnselectedMouseEnter : null },
              ...ranges,
            ];
            return (
              <DayPicker
